...
## NMDS
```{r}
#let's do a NMDS on presence/absence and then use envit
...
envz <- select(zeroenv,2,  3:9, 12, 17, 23, 29, 34)
#wow so many zeros?
...
cwm.env <- select(cov, 1, 2:9, 12, 17, 23, 13, 31, 34)
...
m2 <- capscale(zero~ Prec + esi +mean.cover + dry.veg.percent + shrub.site + Microsite + var.cover + mean.height + Temp + Condition(Site), data = envz, dist = "bray")

m2
summary(m2)
anova.cca(m2, by = "terms", model = "reduced", strata = envz$Site)

m0 <- m1 <- capscale(cwm.comm ~ 1+ Condition(Site), data = cwm.env, dist = "bray")

m1 <- capscale(cwm.comm ~ Prec + esi +mean.cover +dry.veg.percent+  mean.height  + month+ Condition(Site), data = cwm.env, dist = "bray")
...
```{r}
m1 <- glmmTMB(Species ~ dry.veg.percent + Microsite  + mean.cover +  esi + mean.height +  shrub.site + arid + month + veg..height + (1|Site) , family = "poisson", data = cov)
