...
ggplot(fdis, aes(shrub.site, disp)) + geom_boxplot()
ggplot(fdis, aes(arid, esi, color = Site)) + geom_point()
...
m1 <- dis %>% filter(shrub.site != 'open') %>% glm(dis2 ~ Microsite + month + ESI, family = 'gaussian', .)
m1 <- glm(dis2 ~ Microsite+arid  + month , family = 'gaussian',  data = dis)
summary(m1)


ggplot(dis, aes(shrub.site, dis, fill = Microsite)) + geom_boxplot()
