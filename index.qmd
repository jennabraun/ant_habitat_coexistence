...
rda.all <- rda(disp ~ env + mem)
anova.cca(rda(disp, env))
anova.cca(rda(disp, mem))
anova.cca(rda(disp, env, pses))
...
tree2 <- read.tree("moreau/treejb2.nwk")
tree2
plot(tree2)
plot(pruned)
...
labelorder <- c("Pogonomyrmexcalifornicus", "Pheidolehyatti", "Messorpergandei", "Messorandrei", "Solenopsisxyloni", "Cyphomyrmexwheeleri", "Temnothoraxandrei", "Foreliuspruinosis", "Dorymyrmexinsanus", "Dorymyrmexbicolor", "Myrmecocystus")
traitphy <- traitphy[match(labelorder, row.names(traitphy)),]
...
panel.background = element_rect(colour = "black", size=1, fill = NA), axis.line = element_line(colour = "black")) + xlab("Phylogenetic Dispersion (SES MPD)") + ylab("Functional Dispersion (SES FDisp)")
